...
VertexShader =
{
	...
	[[
		...
		{
			VS_OUTPUT Out;
			Out.vPosition  	= mul( ViewProjectionMatrix, float4( v.vPosition.x, 0.0f, v.vPosition.y, 1.0f ) ); // MOD 1.0f raises the borders
			Out.vUV			= v.vUV;
...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			//vAlphaEdge * clamp( PrimaryColor, float4(0.5, 0.5, 0.5, 1.0), float4(0.9, 0.9, 0.9, 1.0)) * 1.9f + ( 1 - vAlphaEdge ) * SecondaryColor;
